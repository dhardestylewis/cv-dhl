...
\vspace{0.6em}
  ...
  \cvitem{8 years}{\textbf{ML Engineering}: Build and operate AI/ML training and inference pipelines, with evaluation suites that catch regressions and improve production reliability}
  ...
  \subsection{\textsc{Professional}}
    ...
    Building a consumer real-estate pricing and forecasting product with interactive uncertainty visualization (fan charts) and leakage-safe walk-forward evaluation; owned end-to-end workflow from data ingestion and training through web delivery.
    \begin{itemize}
        \item Deployed an NYC (Manhattan condos/co-ops) current-price model (12\% holdout error; 8.4\% under an internal apples-to-apples Zillow protocol) as the initial market validation.
        \item Shipped a Houston diffusion-based forecasting system with fan charts; achieved as low as $\sim$8\% annualized compounding error under leakage-safe walk-forward evaluation in best-performing configurations, with deployed selection trading off error versus trajectory realism, delivered through a production web UI: \href{https://houston-price-prediction.vercel.app/}{houston-price-prediction.vercel.app}
        \item Built evaluations, backtests, and model-selection protocols to prevent leakage and support architecture selection across markets.
